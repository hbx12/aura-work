...
| **Discussions Triage & Reply** | `aura-bot-discussions.yml` | `discussion`: created + `discussion_comment`: created |
...
- **Discussion Triage & Reply**: Classifies new discussions and replies to discussion comments using the same language and docs-only policy. It is rate-limited to 4 replies per discussion unless `@hbx12` explicitly mentions `@aura-bot`.
...
3. **Anti-spam**: The bot limits replies to 4 per issue/discussion thread, and only `@hbx12` can override that limit by mentioning `@aura-bot`. It never replies to itself.
...
├── discussion-triage.mjs # Discussion classification & reply
├── discussion-reply.mjs  # Discussion comment replies
