...
function findClient(token: string): BridgeClientConfig | undefined {
  ...
  return config.clients.find((client) => client.sessionToken === hashed);
}
...
const server = createServer(async (req, res) => {
  try {
    ...
    if (
      bridgeRequiresSidecarAuth(method, url) &&
      !isSidecarAuthorized(req, AUTH_TOKEN)
    ) {
    ...
    if (method === "GET" && url === "/health") {
      return json(res, 200, {
        ...
        version: "1.0.0",
        message:
          "Local bridge for Chrome extension, Office add-ins, and CLI companion.",
      });
    }
    ...
    if (method === "POST" && url === "/v1/chrome/page-read/request") {
      ...
      const result = await internalFetch(
        config.internalSecret,
        "/internal/chrome/page-read/request",
        {
          method: "POST",
    ...
    if (method === "POST" && url === "/v1/chrome/page-read/submit") {
      ...
      const result = await internalFetch(
        config.internalSecret,
        "/internal/chrome/page-read/submit",
        {
          method: "POST",
          body: JSON.stringify({ ...body, sessionToken: token }),
        },
