...
fn spawn_sidecar(app: &AppHandle, def: &SidecarDef) -> Option<Child> {
...
fn poll_sidecar_health(port: u16, attempts: u32) -> bool {
    for _ in 0..attempts {
        if ureq::get(&sidecar_health_url(port))
            .call()
...
pub fn start_all(app: &AppHandle) {
    ...
    for def in SIDECARS {
        if let Some(child) = spawn_sidecar(app, def) {
    ...
    std::thread::spawn({
        ...
        move || {
            for def in SIDECARS {
                if poll_sidecar_health(def.port, 40) {
                    ...
                    if poll_sidecar_health(def.port, 20) {
